...
class MCPGateway:
    """Route autonomous agent calls to multiple MCP servers."""

    def __init__(self) -> None:
        self._servers: dict[str, ServerConfig] = {}

    def register_server(self, namespace: str, script_path: Path) -> None:
        """Register one local stdio MCP server."""
    ...
    async def _session(self, config: ServerConfig) -> AsyncIterator[ClientSession]:
        """Start a server and yield an initialized client session."""
...
def build_gateway() -> MCPGateway:
    """Register Phase 3 customer, weather, and ticket servers."""
